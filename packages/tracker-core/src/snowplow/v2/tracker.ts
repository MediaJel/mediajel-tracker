...
import { initialize } from '@mediajel/tracker-core/snowplow/v2/init';
...
const createSnowplowV2Tracker = async (input: CreateSnowplowTrackerInput): Promise<SnowplowTracker> => {
  ...
  const loadEcommerce = async () =>
    (await import("@mediajel/tracker-core/snowplow/v2/ecommerce")).default(input);
  const loadImpressions = async () =>
    (await import("@mediajel/tracker-core/snowplow/v2/impressions")).default();
