...
export const platformSource: Source = ({ widget, emit }) => {
  ...
  emit({
    ...
    summary: detected
      ...
      : `platform: unknown (${globals.join(", ") || "no known globals"})${widget.tag.environment ? ` · tag environment=${widget.tag.environment}` : ""}`,
  });
