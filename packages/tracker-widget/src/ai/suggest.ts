...
import { TimelineEvent, WidgetGoal, WidgetSession } from "@mediajel/tracker-widget/types";
...
const reasonFor = (event: TimelineEvent, goal: WidgetGoal): string | null => {
  ...
  if (event.kind === "network" && event.category === "page") {
    ...
    if (goal === "transaction" && (reading.facts.orderId || reading.facts.total)) {
      return `The site's own server confirmed it: ${event.method} ${new URL(event.url, location.href).pathname} returned an order.`;
    }
