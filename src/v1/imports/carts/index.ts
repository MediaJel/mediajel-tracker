...
export default async (context: QueryStringContext, tracker: SnowplowTracker): Promise<void> => {
  const { appId, environment, retailId } = context;
  switch (environment) {
    //* NEW CART
    case "weave":
      import("./weave").then(({ default: load }): void => load({ appId, retailId }));
    ...
    //* UNSTABLE
    case "dutchieplus":
      import("./dutchie-plus").then(({ default: load }): void => load({ appId, retailId }, tracker));
    ...
    case "wix":
      ...
      break;
    case "sweed":
      import("./sweed").then(({ default: load }): void => load({ appId, retailId }));
