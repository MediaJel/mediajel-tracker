  ...
  return (
    <React.Fragment key={key}>
      {parts.map((p, i) => {
        ...
        if (p.startsWith("`") && p.endsWith("`"))
          return (
            <code key={i} className="rounded-md bg-surface-3/80 px-1.5 py-0.5 font-mono text-[0.85em] text-accent">
              {p.slice(1, -1)}
            </code>
          );
...
export function Objectives({ items }: { items: string[] }) {
  ...
  return (
    <div className="mt-5">
      <div className="mb-2.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-ink-soft">Objectives</div>
...
const Empty = ({ children }: { children: React.ReactNode }) => (
  <div className="grid h-full place-items-center p-6 text-center text-xs text-ink-faint">{children}</div>
);
...
const shortSchema = (s: string | null) => (s ? s.replace("iglu:", "").replace("/jsonschema", "") : "");
...
function SniffRow({ label, detail, highlight }: { label: string; detail?: string; highlight?: boolean }) {
  return (
    <div className="rounded-lg bg-surface-3/50 px-2.5 py-1.5">
      <div className={cx("truncate", highlight ? "text-accent" : "text-ink")}>{label}</div>
      {detail && <div className="truncate text-ink-faint">{detail}</div>}
    </div>
  );
}

/**
 * The exercise inspector adds "network sniffing" to the lesson EventInspector: alongside the
 * Snowplow Micro events (the graded source of truth) it shows the raw DataLayer pushes and
 * Network requests the simulated app emits, plus the sandbox console — so the learner can debug
 * what the app produces vs. what reached the pipeline.
 */
export function ExerciseInspector({
  ...
  const [tab, setTab] = React.useState<"micro" | "datalayer" | "network" | "console">("micro");
  const tabs = [
    { id: "micro", label: "Micro", count: bundle?.counts.total ?? 0 },
    { id: "datalayer", label: "DataLayer", count: dlPushes.length },
    { id: "network", label: "Network", count: netReqs.length },
  ...
  return (
    <div className="flex h-full flex-col">
        ...
        {tabs.map((t) => (
          <button
            ...
            className={cx(
              ...
              tab === t.id ? "bg-surface-3 text-ink" : "text-ink-faint hover:text-ink-soft",
            )}
          >
            {t.label}
            {t.count ? ` · ${t.count}` : ""}
      ...
      <div className="min-h-0 flex-1 overflow-auto p-1 font-mono text-xs">
        {tab === "micro" &&
          (!bundle ? (
            <Empty>Press Start, then interact with the app to capture events.</Empty>
          ) : bundle.good.length === 0 && bundle.bad.length === 0 ? (
            <Empty>No events captured yet — wire up your code and use the app.</Empty>
          ) : (
            <div className="space-y-1">
              {bundle.good.map((e, i) => (
                <EventPill key={"g" + i} good label={e.eventType} sub={shortSchema(e.schema)} />
        ...
        {tab === "datalayer" &&
          (dlPushes.length === 0 ? (
            <Empty>dataLayer pushes from the app appear here.</Empty>
          ) : (
            <div className="space-y-1">
              {dlPushes.map((d, i) => (
                <SniffRow key={i} label={(d && d.event) || "(push)"} detail={truncJson(d)} />
              ))}
            </div>
          ))}
        {tab === "network" &&
          (netReqs.length === 0 ? (
            <Empty>fetch / XHR requests the app makes appear here.</Empty>
          ) : (
            <div className="space-y-1">
              {netReqs.map((r, i) => (
                <SniffRow
                  ...
                  detail={r.body ? truncJson(r.body) : undefined}
                  highlight={/\/api\//.test(r.url)}
                />
              ))}
            </div>
          ))}
...
export function ReportCard({
  ...
  exercises?: { id: string; title: string; icon: string }[];
  exerciseResults?: ExerciseResults;
  onJumpExercise?: (id: string) => void;
}) {
  ...
  return (
    <div className="card overflow-hidden p-6 sm:p-8">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="text-xs font-semibold uppercase tracking-wider text-accent">Report Card</div>
          ...
          <div key={section.id}>
            ...
            <div className="divide-y hairline">
              {section.lessonIds
                ...
                .map((l) => {
                  const s = progress[l.id] as AttemptStat | undefined;
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {s?.completed ? "✓" : lessonIndex(l.id) + 1}
                      </span>
      ...
      {exercises && exercises.length > 0 && (
        <div className="mt-6 border-t hairline pt-5">
          <div className="mb-2 flex items-center justify-between">
            <div className="text-xs font-semibold uppercase tracking-wider text-accent">Exercises</div>
            <span className="text-[11px] tabular-nums text-ink-faint">
              {exercises.filter((e) => exerciseResults?.[e.id]?.completed).length}/{exercises.length} complete
            </span>
          </div>
          <div className="divide-y hairline">
            {exercises.map((ex) => {
              const r = exerciseResults?.[ex.id];
              return (
                <button
                  key={ex.id}
                  onClick={() => onJumpExercise?.(ex.id)}
                  className="flex w-full items-center gap-4 py-3 text-left transition hover:opacity-80"
                >
                  <span
                    className={cx(
                      "grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-bold",
                      r?.completed ? "bg-good text-white" : "bg-surface-3 text-ink-faint",
                    )}
                  >
                    {r?.completed ? "✓" : ex.icon}
                  </span>
                  <span className="flex-1 truncate text-sm text-ink">{ex.title}</span>
                  <span className="hidden text-xs text-ink-faint sm:block">
                    {r?.bestTimeMs != null
                      ? `best ${fmtMs(r.bestTimeMs)}`
                      : r?.attempts
...
export function Stat({ label, value, tone }: { label: string; value: number; tone?: "good" | "bad" }) {
  return (
    <div className="rounded-2xl bg-surface-3/50 p-4 text-center">
      <div
        className={cx("text-3xl font-bold", tone === "good" ? "text-good" : tone === "bad" ? "text-bad" : "text-ink")}
      >
        {value}
      </div>
  ...
  return (
    ...
    >
      <span className="relative flex h-2 w-2">
        {ok && <span className="absolute inline-flex h-full w-full animate-pulse-ring rounded-full bg-good" />}
        <span
          className={cx(
      ...
      <span className="text-ink-soft">{ok === null ? "Checking Micro…" : ok ? "Snowplow Micro" : "Micro offline"}</span>
